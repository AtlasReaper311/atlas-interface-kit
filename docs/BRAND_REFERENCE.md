...
It defines the stable visual identity shared across the estate while preserving the boundary between common foundations and product-owned presentation. The executable token, component, font, and distribution contracts in this repository remain the implementation authority. Accepted architecture decisions in `AtlasReaper311/atlas-infra` take precedence when they supersede this document.

## Authority and ownership

Use this order when deciding interface behaviour:

1. Current files, generated bundle, checks, and tests in `atlas-interface-kit`.
...
`atlas-interface-kit` owns shared tokens, typography assets, component foundations, and deterministic distribution. Consuming repositories own their content, route structure, product-specific controls, data behaviour, deployment, and rollback.
...
Avoid generic dashboard styling, excessive glow, ornamental gradients without information value, and visual effects that reduce readability.
...
| `--text-faint` | Current Interface Kit token | Metadata and low-emphasis labels; must retain validated contrast |
| `--accent` | `#f5a623` | Primary interactive emphasis and Atlas amber identity |
...
The generated Interface Kit token files are authoritative for exact current names and accessibility-adjusted values. Do not copy a stale table into product CSS when the pinned bundle already provides the token.
...
- Warning, preview, and experiment states must remain distinguishable through labels, borders, patterns, or geometry.
...
The approved typefaces are distributed as repository-local WOFF2 assets through the pinned Interface Kit bundle. Each consuming repository must:

1. copy the versioned font stylesheet, font files, licences, and manifest records into its own source tree;
2. verify file sizes and SHA-256 fingerprints using its repository-native checks;
3. serve the assets from its own origin;
4. keep `font-src` restricted to the repository's approved local policy;
...
This distribution model removes cross-domain availability and privacy dependencies while keeping each public surface independently deployable and reversible.

### Typography behaviour

- Display type provides hierarchy and editorial character; it should not be used for controls or dense operational data.
- IBM Plex Mono carries the system voice across navigation, prose, metadata, tables, controls, and evidence.
- Long-form prose must prioritise line length and line height over visual density.
- Uppercase labels use restrained tracking and remain short.
- Italic display emphasis is selective and should not replace structural hierarchy.
...
These are foundations, not instructions to flatten every product into one template. Ramone, Status, API Docs, the CV viewer, the main portfolio, and Lab tools retain purpose-specific composition inside the shared accessibility and navigation contracts.
...
- Dense operational surfaces may use smaller gaps, but controls and readable text retain the minimum target and line-height contracts.
- Cards should not become default containers for every element. Use sections, rails, tables, lists, and direct page composition when they communicate structure more clearly.
...
- Corners remain restrained. Small radii support controls and cards; large soft consumer-product rounding is not part of the core identity.
...
Cards communicate one of four things: a destination, an action, a bounded status summary, or a distinct piece of evidence. Do not combine all four without a clear hierarchy.
...
Labels, marker shapes, border treatment, and text must preserve distinctions for users who cannot rely on colour.
...
Every consuming surface must preserve the current Interface Kit accessibility contract.
...
Motion supports state change and spatial understanding. It does not exist merely to make the interface feel active.
...
- Shared action icons should follow the Interface Kit component contract.
- Product-specific motifs remain in the owning repository.
- Do not introduce a remote icon-font or runtime CDN dependency.
...
- generated image files, source manifest entries, and HTML metadata must remain in sync;
- cache-version changes should accompany an actual visual revision, not routine source churn.

Screenshots and gallery images must have an owning source, accurate alternative text or deliberate decorative treatment, and a publishing path that does not bypass generated-content authority.
...
Visual and asset choices must preserve the consuming repository's security contract.

- Fonts, icons, scripts, styles, and primary images are repository-local unless a reviewed contract permits an external source.
- Content Security Policy should be as restrictive as the product allows.
- Third-party embeds and analytics are not assumed parts of the brand.
- Public metadata must not disclose private infrastructure.
- Browser-facing surfaces should expose the estate's approved `security.txt` contact through their owning deployment.
